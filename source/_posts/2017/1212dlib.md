...
$ ./envsetup
```
`./envsetup`会下载[opencv-android](https://sourceforge.net/projects/opencvlibrary/files/opencv-android/)，这个过程特别慢，建议自己下载最新版本，将zip文件放到`dlib-android/thrid_party/`，并修改envsetup，将函数`def downloadPrebuiltOpencv(prefix='.', out_prefix='.', version='3.3.1')`参数version的默认值改为对应的版本号。
然后再执行`./envsetup`。
