...
# 1.9 可避免偏差
在评估模型的可改进空间时，应结合贝叶斯误差来看待训练集误差和开发集误差。下图中相同的训练集误差和开发集误差，在不同贝叶斯误差的背景下，优化方法会不同。  
此处把人类误差水平等效于贝叶斯误差，把贝叶斯误差和训练误差之间的距离称为**可避免误差**。训练误差和开发误差之间的距离则说明在方差问题上还有多大的改善空间。  
![](0331DeepLearningAI08/img06.png)
前者的可避免误差较大，应该优先解决训练偏差的问题，如扩大网络规模或优化算法。后者可能因为噪声的原因，人类水平的误差就很高，可避免误差很小，因此应该优先解决开发集方差过高的问题，应采用正则化或增大训练集以缓解对训练集的过拟合。

# 1.10理解人的表现
首先应该根据系统的目标定义自己的贝叶斯误差。比如学术研究，要做出超越所有人类的诊断系统，应该将资深医生团队作为贝叶斯误差；如果要实现产品化，做出优于普通医生的可用系统，则应将一般医生的水平作为贝叶斯误差。
![](0331DeepLearningAI08/img07.png)
该目标会直接决定模型的优化方向。第一组应当优先解决可避免偏差较大的问题，第二组则应优先解决训练集方差较大的问题，第三组是最难权衡的，这也是越接近人类越难以提升的一个表现，从绝对值得对比关系来看，还是应当先解决可避免偏差，但是在相差不大的情况下，偏差自身的误差浮动也许已经大于可避免偏差本身了。  
![](0331DeepLearningAI08/img08.png)

# 1.11 超过人的表现
本节粗略地解释了当机器学习超越人的表现之后，为什么继续提升比较困难。
![](0331DeepLearningAI08/img09.png)
第一组的可避免偏差0.1%，方差0.2%，很明显应先解决方差的问题。而第二组训练集误差已超越人类，你无法判断是不是出现了过拟合，即使没有过拟合，接下来贝叶斯偏差线在哪里，已经没有判断依据了，因此先解决可避免偏差还是方差也就失去了判断依据。优化失去方向，这是进一步提升的难点。  

不过在很多方面，机器学习已经超越了人类，如：在线广告、相关产品推荐、物流预测、贷款风控等。人类擅长更直观的自然感知，如识图、听声。不过即使在这些领域，机器学习也正在超越人类。

# 1.12 改善你的模型表现
本节是对以上内容的总结，完成监督学习的两个基本要点：
1. 让模型很好地拟合训练集达到低偏差
2. 让模型很好地拟合开发集达到低方差

在正交化的思想下分别达成两个目标。在量化效果时拉出三条线：
![](0331DeepLearningAI08/img10.png)
如果可避免偏差过大则应使用规模更大的模型或更换更优化的算法，比如加入momentum或RMSprop或Adam或寻找更好的神经网络架构或优化更好的超参数。  
如果方差较大，则应为训练集收集更多的数据或使用正则化包括L2正则化、Dropout正则化，也可以尝试使用不同的神经网络架构、超参数搜索等。